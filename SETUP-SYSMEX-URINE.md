...
| Reported words | `testValueMap` on the `sysmex-uwam` profile: `-` → **Absent** (C-PRO, C-GLU, C-KET, C-BIL), **Negative** (C-NIT), **Normal** (C-URO, which arrives as the word `normal`); `+-` → **trace** (C-PRO, C-GLU); C-NIT `+` → **Positive**; `1+`…`4+` pass through; C-BLD, C-LEU, C-COLOR, C-PH, S.G. filed as sent; C-CLOUD not synced | **Lab specification**, 2026-09-18 (pinned in `test/uwam-value-map.test.ts`). Applied at delivery time, so the spool is repaired by a corrected table. Strictly the U-WAM link |
...
| Ignore list | `*Info*` (RBC-Info. / UTI-Info. / BACT-Info.), `*?`, `C-Error Code`, `C-ColorRANK`, unregistered research parameters; on the **U-WAM link also `C-CLOUD`** (turbidity — not interfaced, its HMIS row is filled by other means, lab 2026-09-18) | **From capture + manual** (GI §8.1.1, §5.6.4). `Lysed RBC` and `SRC` are research items but HMIS registers them, so they are NOT ignored |
